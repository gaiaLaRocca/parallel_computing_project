...
per il block --- la stessa cosa entro il rumore di misura. Non c'è stato alcun
dialogo aggiuntivo: solo la medesima gather finale.

\paragraph{Il master--worker e l'effetto coda.}
...
$2{,}88$ della baseline seriale. Il residuo del $2{,}8\%$ ha una spiegazione
strutturale: è l'\emph{effetto coda}. L'ultima riga consegnata a un worker può
essere la più pesante della griglia, e con $\lambda_{\text{riga}} = 2{,}76$ la
perdita di coda vale al più $(\lambda_{\text{riga}} - 1)\,p/H$, cioè il $5{,}5\%$
a $p=32$:
il $2{,}8\%$ misurato sta dentro quel margine. Sul fronte del coordinamento, il
master resta lontano dalla saturazione: a $p=32$ un worker spende circa $18~\mu$s
di round-trip per riga, contro $0{,}70$~ms di calcolo medio: circa quaranta volte
meno.
...
Il deficit del cyclic ad alto $p$ non è imputabile né a $\lambda_p$, che è quasi
$1$, né alla gather, che è la stessa del block. Il candidato è il riordino di
de-interlacciamento discusso sopra: una copia seriale su rank $0$ dell'intera
matrice, $4$~MiB letti e $4$~MiB scritti, di costo indipendente da $p$ e
stimabile attorno al millisecondo. A $p=32$ il divario dal master--worker è di
$0{,}95$~ms, coerente con la stima; a $p=16$ è invece di $3{,}1$~ms, tre volte
tanto, e un costo fisso non basta a spiegarlo. Poiché $p=16$ è anche il punto con
la dispersione più alta, l'attribuzione resta un'ipotesi: la verifica diretta,
cronometrando il riordino o eliminandolo con il tipo derivato
\texttt{MPI\_Type\_vector}, rientra fra gli sviluppi del lavoro.

\paragraph{La comunicazione non è il collo di bottiglia.}
In termini assoluti la comunicazione è irrilevante in tutta la campagna: fra
...
calcolo contro $18~\mu$s di protocollo) il messaggio non è il fattore limitante.
...
decomposizioni riproducono la matrice degli escape time bit per bit, su conteggi
di rank che dividono e non dividono l'altezza, e nessuno schema ha alterato il
risultato. Le sei baseline seriali a $1024^2$ e $N_{\max}=1000$ registrate nel
