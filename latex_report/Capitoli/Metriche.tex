...
costo. Il suo peso non è stato cronometrato separatamente, ed è l'unica
grandezza del modello a restare stimata: la matrice di riferimento occupa
$1024^2$ interi a 32~bit, cioè 4~MiB, il cui azzeramento costa fra il decimo di
millisecondo e il millisecondo --- a seconda che l'allocatore riusi pagine già
mappate o ne chieda di nuove al kernel --- contro i $722$~ms del calcolo. Il
confronto con i tempi paralleli misurati restringe l'intervallo verso il basso
(sezione~\ref{sec:parallelizzazione}), ma la misura diretta resta fra gli
sviluppi del lavoro.
...
\]
Come discusso nella sezione~\ref{sec:parallelizzazione}, nel codice in esame
$1-P$ è dell'ordine di $10^{-4}$: Amdahl prevede uno speedup quasi ideale e
fornisce il \emph{limite di riferimento}, ma non spiega gli scostamenti
osservati. Il valore non è misurato direttamente, e la
sezione~\ref{sec:parallelizzazione} mostra entro quali margini i dati paralleli
lo vincolino.
