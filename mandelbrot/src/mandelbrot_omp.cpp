...
// OpenMP kernel. Identical arithmetic to the serial baseline (mandelbrot.cpp):
// only the outer row loop is parallelised, so the escape-time matrix - and thus
// the checksum - is bit-for-bit the same. The helper functions below are copied
// verbatim from the serial kernel on purpose: reproducing them keeps the
// per-pixel computation, and therefore validation, identical.
...
Paradigm kernel_paradigm() { return Paradigm::OpenMP; }
...
MandelbrotImage compute_mandelbrot(const Viewport& view, int width, int height,
    ...
    // Parallelise over rows, not pixels. Each thread owns whole contiguous rows
    // (>= one cache line each), so distinct threads write to distant memory
    // regions and there is no false sharing; the per-iteration scheduling cost
    // is amortised over `width` pixels instead of one. Loop-local `c_imag`,
    // `col` and `c_real` are private by construction; every thread writes only
    // the cells of its own rows, so no race on img.data.
    //
    // schedule(runtime) defers the policy to the OMP_SCHEDULE environment
    // variable, so a single binary sweeps static / dynamic / guided without a
    // recompile (the job script sets OMP_SCHEDULE and the matching --schedule
    // label).
